...
NASM__ = r'D:\\Verano-Interfaz\\pyAsm\\nasm.exe'
...
class pyAsm():
    ...
    def run(self):
        ...
        if bytes is not None and SignalAddress is not None:
            ...
            thread_id = c_ulong(0)
            h_thread = windll.kernel32.CreateThread(
                None, 0, ptr, None, 0, byref(thread_id))
    ...
    def _addClosure(self):
        ...
        # Set flag to 0 - Start
        self.update('mov dword [' + hex(ptrHeap) + '],' +
                    hex(THREAD_START_SIGNAL), START)
        self.update('__LP__:')
        self.update('mov dword [' + hex(ptrHeap) + '],' +
                    hex(THREAD_FIN_SIGNAL))  # Set flag to 1 - End
    ...
    def _PlaceDataInMemory(self, data, BaseAddress=0x400000):
        ...
        ptr = 0
        while ptr == 0:
            ptr = windll.kernel32.VirtualAlloc(baseAddr, len(
                data)*2, MEM_COMMIT, PAGE_EXECUTE_READWRITE)
